...
data = {
    'MOT17': {
        ...
        'test':[
           r'Path to the directory with the video frames'
...
class opts:
    def __init__(self):
        ...
        self.parser.add_argument(
            '--root_dataset',
            #
           default = '/mnt/gpid08/users/javier.de.fuentes/process_video_for_tracking'
        )
        self.parser.add_argument(
            '--path_AFLink',
            default=r'Path to AFLink weights'
        )
        self.parser.add_argument(
            '--dir_save',
             default =r'Path to the results folder'
        )
        self.parser.add_argument(
            '--EMA_alpha',
            default=0.9
    ...
    def parse(self, args=''):
        ...
        if opt.BoT:
            ...
            opt.dir_dets = r'Path to the feature vectors of the video'
        else:
            opt.max_cosine_distance = 0.3
        
        if opt.MC: #Matching with both appearance and motion costaids association
            opt.max_cosine_distance += 0.05
        if opt.EMA: #Updating strategy to leverage the info of inter-frame feature changes and can depress detection noise
            opt.nn_budget = 1
        else:
            opt.nn_budget = 100
        if opt.ECC: #Camera movement-> We are not using this for traffic
